...
import com.restonic4.ae2redstone.screens.EnergyConverterScreen;
...
public class AE2RedstoneClient implements ClientModInitializer {
    ...
    public static void openConverterScreen(BlockPos pos) {
        Minecraft mc = Minecraft.getInstance();
        if (mc.level != null) {
            BlockEntity be = mc.level.getBlockEntity(pos);
            if (be instanceof EnergyConverterBlockEntity converter) {
                mc.setScreen(new EnergyConverterScreen(converter));
            }
        }
    }
}